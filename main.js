  ...
  chat(messages, tools) {
    return __async(this, null, function* () {
      ...
      const payload = {
        model: this.model,
        max_tokens: 4096,
        // Reasonable limit - truncation is now handled gracefully
      ...
      if (!response) {
        throw new Error(`Anthropic API request failed: ${(lastError == null ? void 0 : lastError.message) || "Unknown error"}`);
      }
    ...
    return __async(this, null, function* () {
      ...
      switch (name) {
        case "search_omnisearch":
          ...
          const omnisearchResults = yield this.vaultManager.searchWithOmnisearch(args.query);
          return {
        ...
        case "create_note":
          const created = yield this.vaultManager.createNote(args.path, args.content);
          return {
            success: created,
            path: args.path,
        ...
        case "list_commands":
          ...
          if (args.filter) {
            ...
            return allCommands.filter(
              (cmd) => cmd.id.toLowerCase().includes(filterLower) || cmd.name.toLowerCase().includes(filterLower)
            );
          }
          return allCommands;
        case "execute_command":
          const executed = yield this.vaultManager.executeCommand(args.command_id);
          return {
            success: executed,
...
var VaultManager = class {
  ...
  getVaultOverview() {
    ...
    const recentFiles = files.sort((a, b) => b.stat.mtime - a.stat.mtime).slice(0, 20).map((f) => f.path);
    return {
   ...
   */
  searchWithOmnisearch(query) {
    return __async(this, null, function* () {
      var _a, _b;
      try {
        const omnisearch = (_b = (_a = this.app.plugins) == null ? void 0 : _a.plugins) == null ? void 0 : _b["omnisearch"];
        if (!omnisearch) {
          return [];
        }
  ...
  isOmnisearchAvailable() {
    var _a, _b;
    return !!((_b = (_a = this.app.plugins) == null ? void 0 : _a.plugins) == null ? void 0 : _b["omnisearch"]);
  }
  /**
  ...
  executeCommand(commandId) {
    return __async(this, null, function* () {
      try {
        yield this.app.commands.executeCommandById(commandId);
        return true;
      } catch (error) {
  ...
  handleUserMessage(content) {
    return __async(this, null, function* () {
      if (this.isProcessing) {
        yield this.addSystemMessage("\u23F3 Please wait for the current response to complete...");
        return;
      }
  ...
  generateNote(template, data) {
    ...
    for (const [key, value] of Object.entries(data.frontmatter)) {
      if (Array.isArray(value)) {
        ...
        frontmatterLines.push(`${key}: ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}`);
      } else {
...
`);
  ...
  startTemplateInterview(templateName) {
    return __async(this, null, function* () {
      ...
      if (leaf && leaf.view instanceof ChatView) {
        leaf.view.interviewTemplate = template;
        const openingMessage = this.templateManager.getInterviewPrompt(template);
        yield leaf.view.addSystemMessage(openingMessage);
      }
    });
  }
    ...
    return __async(this, null, function* () {
      ...
      while (iterations < MAX_ITERATIONS) {
        ...
        if (response.stop_reason === "max_tokens") {
          yield view.addSystemMessage("\u26A0\uFE0F Response was truncated. Reply 'continue' for more.");
        }
        if (response.content || response.tool_calls) {
          let content = response.content || "";
          const appearsTruncated = content && (!content.trim().match(/[.!?]$/) && // Doesn't end with punctuation
  ...
  getSystemPrompt(ctx, view) {
    if (view && view.interviewTemplate) {
      const template = view.interviewTemplate;
